...
public class ListarContrato extends JDialog {
	...
	//panel.setBorder(new TitledBorder(null, "Listado de planes", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		String[] columnNames = {"#","Estado","Precio del Plan"};
		tableModel.setColumnIdentifiers(columnNames);
		loadListaFacturas(1);
		scrollPane.setViewportView(table);
		
		/*JLabel lblTipoDePlan = new JLabel("Tipo de Contrato:");
		lblTipoDePlan.setBounds(8, 14, 90, 14);
		panel.add(lblTipoDePlan);
		
		cbxContrato = new JComboBox();
		cbxContrato.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int selection = cbxContrato.getSelectedIndex();
				loadListaFacturas(selection);
			}
		});
		cbxContrato.setModel(new DefaultComboBoxModel(new String[] {"Todas", "Activa", "No Activa"}));
		cbxContrato.setBounds(120, 11, 110, 20);
		panel.add(cbxContrato);*/
		
		JLabel lblNewLabel = new JLabel("Listado de Contratos");
	...
	public static void loadListaFacturas(int selection) {
		...
		case 0:
			...
			for(Cliente cli:Tricom.getInstance().getMiCliente()) {
				
				for(Contrato cont: cli.getMiscontract()) {
				
					fila[1] = cont.getCodigoDeContrato();
					fila[0] = cont.getEstado();
					fila[2]= cont.getPrecioDelPlan();
				}
			}
			/*for (Contrato aux: Tricom.getInstance().getMisContrato()) {
					fila[0] = aux.getCodigoDeContrato();
					fila[1] = aux.getEstado();
					fila[2]= aux.getPrecioDelPlan();
